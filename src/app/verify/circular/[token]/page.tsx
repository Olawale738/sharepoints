...
export default async function CircularVerificationPage({ params }: { params: Promise<{ token: string }> }) {
        ...
        <div className="bg-[#0b1b3d] px-6 py-6 text-white">
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <h1 className="mt-2 text-2xl font-semibold">Official Circular Verification</h1>
            </div>
          </div>
        </div>
        <div className="p-6">
            ...
            <div className="flex items-start gap-3">
              ...
              <div>
                ...
                <p className="mt-2 text-sm leading-6 text-ink/60">
                  This page confirms whether a circular is a current LETW instruction. It does not expose restricted internal attachments, private branch notes, or protected acknowledgements.
                </p>
              </div>
            </div>
          </div>
          <div className="mt-6 grid gap-3 sm:grid-cols-2">
            <Info label="Circular title" value={result.title} />
            <Info label="Seal number" value={result.sealNumber ?? "Not recorded"} />
            <Info label="Current status" value={result.status ?? "not found"} />
            <Info label="Audience" value={result.scope ?? "LETW official registry"} />
            <Info label="Issued" value={result.issuedAt ? formatDate(new Date(result.issuedAt)) : "Not issued"} />
            <Info label="Expires" value={result.expiresAt ? formatDate(new Date(result.expiresAt)) : "No expiry recorded"} />
            <Info label="Revoked" value={result.revokedAt ? formatDate(new Date(result.revokedAt)) : "No"} />
          </div>
